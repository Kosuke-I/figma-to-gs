 ...
 */
type StylesMap = { [styleType in StyleType]: string };
...
type Effect = { type: EffectType } & Effect_ & Partial<EffectShadow_>;

function isEffectShadow(effect: Effect): effect is EffectShadow {
  return (
...
function isEffectBlur(effect: Effect): effect is EffectBlur {
  return (
...
type Paint = { type: PaintType } & Paint_ &
  Partial<PaintSolid_> &
  Partial<PaintGradient_> &
  Partial<PaintImage_>;

function isPaintSolid(paint: Paint): paint is PaintSolid {
  return paint.type === PaintType.SOLID;
}

function isPaintGradient(paint: Paint): paint is PaintGradient {
  return (
...
function isPaintImage(paint: Paint): paint is PaintImage {
...
/** The root node */
interface DOCUMENT_NODE {
  /** An array of canvases attached to the document */
  children: NodeViewing[];
}
...
interface CANVAS {
  ...
  /** default: [] An array of settings representing images to from the canvas */
...
interface FRAME {
  ...
  /** default: [] An array of settings representing images to from node */
  exportSettings: ExportSetting[];
  /** How this node blends with nodes behind it in the scene (see blend mode section for more details) */
  blendMode: BlendMode;
...
type NodeViewing<NType extends NodeType = NodeType> = {
  id: string;
  name: string;
  visible: boolean;
  type: NType;
...
function isNodeType<NType extends NodeType, R = NodeViewing<NType>>(
  node: NodeViewing<any>,
  type: NType
): node is R {
